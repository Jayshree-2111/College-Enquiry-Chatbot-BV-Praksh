...
if (isset($_POST["signup"])) {
  ...
  $check_email = mysqli_num_rows(mysqli_query($conn, "SELECT bv_emailid FROM student_register WHERE bv_emailid='$bv_emailid'"));
  ...
  if ($check_email > 0) {
    ...
    $sql = "INSERT INTO student_register (bv_id, bv_emailid, pass) VALUES ('$bv_id', '$bv_emailid', '$pass')";
...
if (isset($_POST["send"])) {
  ...
  $all = mysqli_query($conn, "SELECT * FROM student_register WHERE bv_id='$id' AND pass='$dob'");
  ...
  if($count > 0){
    ...
    $mail->Body="<p>Dear student, </p><h3>Your OTP verification code is: $otp <br></h3>
    <br/>
    ...
    if(!$mail->send()){
      ...
      <script>
        ...
        window.location.replace('stu_otp.php');
      </script>
      <?php
    }            
  }else{
...
<head>
  ...
  <title>Student Log in & Register Form</title>
</head>
      ...
      <div class="signin-signup">
        <form action="" class="sign-in-form" method="post">
          ...
          <div class="input-field">
            ...
            <input type="text" placeholder="Student ID" name="bv_id" pattern="([A-Z]{1,5})?([0-9]{1,5})" minlength="10" title="Please enter correct Student ID in capital letters" value="<?php echo $_POST['bv_id']; ?>" required />
          </div>
          <div class="input-field">
            <i class="fas fa-lock"></i>
            <input type="password" onfocus="(this.type='date')" placeholder="Date of Birth" name="bv_dob" min="1990-01-01" max="2012-12-31" title="Please enter a valid Date Of Birth only" value="<?php echo $_POST['bv_dob']; ?>" required />
          </div>
        ...
        <form action="" class="sign-up-form" method="post">
          ...
          <div class="input-field">
            <i class="fas fa-user"></i>
            <input type="text" placeholder="Student ID" name="signup_bv_id" pattern="([A-Z]{1,5})?([0-9]{1,5})" minlength="10" title="Please enter correct Student ID in capital letters"  value="<?php echo $_POST["signup_bv_id"]; ?>" required />
          </div>
          <div class="input-field">
            <i class="fas fa-envelope"></i>
            <input type="email" placeholder="Banasthali Email" name="signup_bv_emailid" pattern=".+@banasthali\.in" title="Please enter a valid Banasthali email id only" size="50" value="<?php echo $_POST["signup_bv_emailid"]; ?>" required />
          </div>
          <div class="input-field">
            <i class="fas fa-lock"></i>
            <input type="password" onfocus="(this.type='date')" placeholder="Date of Birth" name="signup_password" min="1990-01-01" max="2012-12-31" title="Please enter a valid Date Of Birth only" value="<?php echo $_POST["signup_password"]; ?>" required />
          </div>
